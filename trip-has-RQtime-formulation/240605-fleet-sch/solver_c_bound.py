...
c = abs(v/2)
#loop
while (end - start < TIME_LIMIT and min < max):
    command = f"clingo path_edge_weight.lp init.lp opt_bound.lp network.lp rq.lp mer_lmp.lp -q0 -t{THREAD} --outf=0 -c timestep={TIMESTEP} -c bound={c}"
    output_from_terminal = subprocess.run(command, capture_output=True, text=True, shell=True)
